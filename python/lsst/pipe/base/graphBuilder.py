        ...
        *,
        ...
        for task in self.tasks:
            ...
            for quantum in task.quanta.values():
                ...
                # Resolve all task inputs and outputs.
                if idMaker:
                    for datasetType in task.inputs:
                        resolved = idMaker.resolveDict(quantum.inputs[datasetType])
                        quantum.inputs[datasetType].update(resolved)
                    for datasetType in task.outputs:
                        resolved = idMaker.resolveDict(quantum.outputs[datasetType])
                        quantum.outputs[datasetType].update(resolved)
                    for inits in (task.initInputs, task.initOutputs):
                        for datasetType in inits:
                            resolved = idMaker.resolveDict(inits[datasetType])
                            inits[datasetType].update(resolved)
